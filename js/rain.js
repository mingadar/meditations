...
const rainMeditationTimer = new MeditationTimer(
    document.querySelector('#player'),
    698, // 11 minutes and 37 seconds in seconds
    document.getElementById('timer'),
  );

  rainMeditationTimer.audioElement.addEventListener('play', () => {
    rainMeditationTimer.start();
  });
  
  rainMeditationTimer.audioElement.addEventListener('pause', () => {
    rainMeditationTimer.stop();
  });

  rainMeditationTimer.audioElement.addEventListener('timeupdate', () => {
    if (!rainMeditationTimer.audioElement.paused) {
      rainMeditationTimer.start();
    }
  });
